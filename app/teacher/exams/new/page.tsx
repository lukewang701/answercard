...
export default function NewExamPage() {
  ...
  const [formData, setFormData] = useState({
    name: '',
    classes: '', // Single class ID or name (API still extracts 'classes' field)
  ...
  useEffect(() => {
    fetch('/api/classes').then(r => r.json()).then(data => {
      setClassesList(data);
      if (data.length > 0) {
        setSelectedClasses([data[0].name]);
      }
    });
  }, []);

  useEffect(() => {
    setFormData(prev => ({ ...prev, classes: selectedClasses.join(',') }));
  }, [selectedClasses]);
  ...
  const [fastInputs, setFastInputs] = useState<string[]>(Array(15).fill(''));
  ...
  const handleFastInputChange = (index: number, value: string) => {
    // Only allow A-E, 1-5, Space, and Slash
    ...
    if (validTokens.length > expectedTokens) {
      ...
      mappedValue = validTokens.join(' ') + ' '; // keep the trailing space to allow jump
    }
    ...
    // Auto advance to next input if we have the expected number of tokens and hit space
  ...
  const toggleAnswer = (qIndex: number, opt: string) => {
    setQuestions(prevQ => {
      ...
      // Remove trailing empty strings and join with space
  ...
  const handlePointKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, qIndex: number) => {
    // If they press a digit key (0-9)
    if (/^[0-9]$/.test(e.key)) {
      e.preventDefault();
      updatePoints(qIndex, e.key);
      // Auto-jump to next question's point input
  ...
  const handleScanComplete = (result: any) => {
    if (result && result.answers) {
      ...
      // Auto-collapse scan and expand click to review
    ...
    try {
      ...
      if (res.ok && data.success) {
        if (data.exams && data.exams.length > 1) {
          // If multiple exams were created, redirect to dashboard
          router.push('/teacher');
        } else {
          router.push(`/teacher/exams/${data.exams[0].id}`);
        }
      } else {
      ...
      <form onSubmit={handleSubmit}>
        <div className="card mb-8">
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            ...
            <div>
              <label className="block mb-2 text-sm">套用班級 <span className="text-xs opacity-70">(可複選，將自動建立多份試卷)</span></label>
              <div className="flex flex-wrap gap-3 max-h-[88px] overflow-y-auto p-2 border border-border rounded bg-background">
                {classesList.map(c => (
                  <label key={c.id} className="flex items-center gap-2 cursor-pointer hover:bg-secondary/20 px-2 py-1 rounded">
                    <input 
                      type="checkbox" 
                      ...
                      onChange={(e) => {
                        if (e.target.checked) {
                          ...
                          setSelectedClasses(prev => prev.filter(name => name !== c.name));
                        }
                      }}
                    />
                ...
                {classesList.length === 0 && <span className="text-sm opacity-50 px-2">無班級資料</span>}
              </div>
            </div>
            <div>
          ...
          <p className="text-sm text-warning m-0">註：若題目未設定配分，系統將自動依剩餘總分平均分配。</p>
        </div>
